                ...
                <div id="table" class="table-editable">
                    ...
                    <table class="table table-bordered table-responsive-md table-striped text-center">
                        ...
                        <tbody>
                            ...
                            @if (count($membership->membershipFeatures) > 0)
                                @foreach ($membership->membershipFeatures as $membershipFeature)
                                    <tr>
                                        ...
                                        <td contenteditable="true"><input type="text" class="form-control"
                                                ...
                                                value="{{ old('description', $membershipFeature?->pivot->description) }}">
                                        </td>
                                        <td contenteditable="true"><input type="text" class="form-control"
                                                name="{{ $membershipFeature->id }}-url"
                                                id="{{ $membershipFeature->id }}-url"
                                                value="{{ old('url', $membershipFeature?->pivot->url) }}"></td>
                                        <td contenteditable="true"><select class="form-control"
                                                name="{{ $membershipFeature->id }}-has_access"
                                                id="{{ $membershipFeature->id }}-has_access"
                                                value="{{ old('has_access', $membershipFeature?->pivot->has_access) }}">
                                                <option value="true"
                                                    @if ($membershipFeature->pivot->has_access == 1) selected @endif>Con
                                                    Acceso</option>
                                                <option value="false"
                                                    @if ($membershipFeature->pivot->has_access == 0) selected @endif>Sin
                                                    Acceso</option>
                                            </select> </td>
                                        <td contenteditable="true"><input type="text" class="form-control"
                                                name="{{ $membershipFeature->id }}-usage_limit"
                                                id="{{ $membershipFeature->id }}-usage_limit" placeholder="null"
                                                value="{{ old('usage_limit', $membershipFeature?->pivot->usage_limit) }}">
                                        </td>
